	...
	<div class="add">
      ...
      <div class="question_list">
        ...
        <ul >
          ...
          <li>
            <el-button type="primary" class="button" @click="addNewFormItem('Q&A')">简答题或填空题</el-button>
          </li>
        </ul>
      </div>
        ...
        <ul>
          <li v-for="(item,index) in form._questions" :key="item.id" class="marginB10">
            <p class="question-title">{{index+1}}、({{item.type=='single'?'单选题'
              :item.type=='multi'?'多选题':item.type=='judgement'?'判断题':item.type=='Q&A'?'简答题或填空题':'null'}}){{item.name}} ({{item.score}}分)
